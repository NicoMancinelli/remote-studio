...
Make sure all tests compile successfully. Tests should execute, verify outputs, and fail cleanly when run against the current unimplemented/stub `res` binary. Verify the compilation and test run by running `go test ./tests/e2e/...` inside the subagent workspace, and report the test run results back.
