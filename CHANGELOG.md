...
- **`res status` exposes RustDesk FPS and bitrate.** Two new fields (`fps`,
  ...
  compact snippet (`30f·2Mbps`) in the panel label and as separate lines
  in the tooltip, only while a session is open. Helpers live in
  `lib/services.sh` so `res rustdesk status` and `res status` agree.
